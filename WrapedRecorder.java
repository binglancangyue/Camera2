...
//end
public class WrapedRecorder implements SensorEventListener, AWRecorder.OnErrorListener, AWRecorder.OnInfoListener {
	...
	// end
	private static final boolean IS_BACK_RIGHT_ON = SystemProperties.getBoolean("ro.sys.back.right", true);
	...
	private Handler mHandler = new Handler(Looper.getMainLooper()) {
		public void handleMessage(Message msg) {
			...
			case MSG_OPEN_CAMERA_CB:
				Log.d(TAG, "MSG_OPEN_CAMERA_CB msg.arg1=" + msg.arg1);
				if (msg.arg1 == 1) {
					/*
					...
					if (mTryTime < TRY_MAX_TIME) {
						if (mRecorderHandler != null) {
							...
							mRecorderHandler.sendEmptyMessageDelayed(MSG_OPEN_CAMERA, SEND_MSG_DELAY);
						}
					} else {
						mTryTime = 0;
					...
					Log.d(TAG, "MSG_OPEN_CAMERA_CB mCamera=" + mCamera);
			...
			case MSG_GSENSOR_LOCK_UP:
				 ...
				 * LOCK_FLAG_NONE; }
				 */
				ActivityManager manager = (ActivityManager) mRecordService.getSystemService(Context.ACTIVITY_SERVICE);
				...
				if (!namel.equals("com.android.camera2") && !namer.equals("com.android.camera2")) {
	...
	public synchronized String getCanRepalceFiles() {
		...
		if (file_test.exists()) {
			...
			Log.d(TAG,
					"count_front = " + count_front + " count_back = " + count_back + " count_third = " + count_third);
		...
		if (!checkNeedSwitchNow()) {
			...
			if (new File(old).exists()) {
				...
				if (new File(videoPath).exists()) {
					...
					Log.d(TAG, "mCameraId = " + mCameraId + "  new File existes ftruncate result =" + result);
				}
			}

			if (result < 0 || !(new File(videoPath).exists())) {
				result = mRecorder.pre_qc_fallocate(videoPath, getFallocateSize());
				Log.d(TAG, "mCameraId = " + mCameraId + "  ftruncate fail todo fallocate pre result =" + result);
			}
		}
	}
	...
	private class RecorderHandler extends Handler {
		...
		public void handleMessage(Message msg) {
			...
			case MSG_OPEN_CAMERA:
				...
				Log.d(TAG, "MSG_OPEN_CAMERA mCameraId =" + mCameraId);
				Log.d(TAG, "mRecorderHandler =" + mRecorderHandler);
				Log.d(TAG, "WrapedRecorder =" + WrapedRecorder.this);
				Log.d(TAG, "mHandler=" + mHandler + ";mCameraOpenErrorCallback=" + mCameraOpenErrorCallback);
				...
				if (cameraIndex >= 0) {
					try {
						...
						Log.d(TAG, "handleMessage RuntimeException = mCamera---=" + mCamera + "----cameraIndex=   "
								+ cameraIndex);
						ex.printStackTrace();
					}
				}
				mParameters = null;
				Log.d(TAG, "MSG_OPEN_CAMERA mCamera=" + mCamera);
				...
				if (mRecorder != null && mCamera != null) {
					...
					try {
						...
						if (mMiniMode == MINI_MODE_WAITING_TAKEN && mMiniVideoTakenListener != null) {
							...
							Log.d("cdong", "mMiniWidth = " + mMiniWidth + "mMiniHeight = " + mMiniHeight);
							mCurVideoValues = generateVideoFilename(AWRecorder.OutputFormat.MPEG_4);
						} else {
							 ...
							 */
							Log.d("cdong", "mDesiredPreviewWidth_src = " + mDesiredPreviewWidth_src
									+ " mDesiredPreviewHeight_src = " + mDesiredPreviewHeight_src);
							Log.d("cdong", "mDesiredPreviewWidth = " + mDesiredPreviewWidth
							...
							if (mCameraId == CameraInfo.CAMERA_FACING_FRONT && mDesiredPreviewHeight_src == 720
								...
								// 720);
								mRecorder.setVideoSize(mDesiredPreviewWidth_src, mDesiredPreviewHeight_src,
										mDesiredPreviewWidth, mDesiredPreviewHeight);
							} else {
								mRecorder.setVideoSize(mDesiredPreviewWidth, mDesiredPreviewHeight);
							}
							...
							mCurVideoValues = generateVideoFilename(AWRecorder.OutputFormat.OUTPUT_FORMAT_MPEG2TS);
							// todo
			...
			case MSG_STOP_RECORD:
				...
				if (mCameraId == CameraInfo.CAMERA_FACING_FRONT) {
					if (mRecordService.isSDOut()) {
						...
						if (mRecorderHandler != null) {
							mRecorderHandler.removeMessages(MSG_WRITE_KML);
							mRecorderHandler.sendEmptyMessageDelayed(MSG_WRITE_KML_END, TICKY_DELAY);
						}
					}
				}
				mLockFlag = LOCK_FLAG_NONE;
			...
			case MSG_SWITCH_NEXT_FILE:
				if (mRecorder != null && mCamera != null) {
					...
					if (mForceLock) {
						mForceLock = false;
						Log.d(TAG, "MSG_SWITCH_NEXT_FILE " + mCurVideoValues.getAsString(Video.Media.DATA));
						generateImpactVideoFilename(mCurVideoValues);
						Log.d(TAG, "MSG_SWITCH_NEXT_FILE " + mCurVideoValues.getAsString(Video.Media.DATA));
			...
			case MSG_SET_MUTE:
				if (mRecorder != null) {
					Log.d(TAG, "setMicMute msg.arg1=" + msg.arg1);
					if (mCameraId == CameraInfo.CAMERA_FACING_FRONT) {
			...
			case MSG_START_PREVIEW:
				if (mCamera != null) {
					 ...
					 * sizes.get(0).height);
					 */
					Log.d(TAG, "mDesiredPreviewWidth =" + mDesiredPreviewWidth + "mDesiredPreviewHeight="
			...
			case MSG_SET_SURFACE_HOLDER:
				...
				if (mCamera != null && msg.arg1 < TRY_MAX_TIME) {
					...
					Log.d(TAG, "msg.arg1 =" + msg.arg1 + ", surface = " + surface + "   isValid: " + surface.isValid());
					try {
			...
			case MSG_SET_ADAS_SENSITY:
				if (mCamera != null) {
					...
					mParameters = mCamera.getParameters();
					if (mParameters != null) {
						Log.d(TAG, String.format("--------  onDistanceDetectChange state %d------- ", msg.arg1));
	...
	private void getDesiredPreviewSize(boolean isRestartPreview) {
		...
		if (!isSupport && maxIndex >= 0 && maxIndex < sizes.size()) {
			Log.d(TAG, " get default max size mDesiredPreviewWidth =" + mDesiredPreviewWidth + ";mDesiredPreviewHeight"
					+ mDesiredPreviewHeight);
			if (mCameraId == CameraInfo.CAMERA_FACING_FRONT) {
	...
	public void onSensorChanged(SensorEvent sensorEvent) {
		...
		if (sensorEvent.sensor.getType() == Sensor.TYPE_ACCELEROMETER) {
			...
			if ((xlateral > threshold) || (ylongitudinal > threshold) || (zvertical > threshold)) {
				Log.v(TAG, "heading=" + xlateral + ", pitch=" + ylongitudinal + ", roll=" + zvertical + ", threshold="
						+ threshold);
	...
	private void generateImpactVideoFilename(ContentValues values) {
		if (values != null) {
			...
			if (!oldPath.contains(IMPACT_SUFFIX)) {
				String path = oldPath.substring(0, index) + IMPACT_SUFFIX + oldPath.substring(index);
	...
	public void saveVideo(ContentValues conVal) {
		...
		mMediaSaveService.addVideo(finalFile, duration, conVal, mOnVideoSavedListener, mContentResolver);
	}
	...
	public void updatVideoName(ContentValues conVal, String fileName) {
		...
		mMediaSaveService.addVideo(fileName, duration, conVal, mOnVideoSavedListener, mContentResolver, true);
	...
	private void storeImage(final byte[] data, Location loc) {
		...
		if (Storage.getTotalSpace() > 0) {
			mMediaSaveService.addImage(data, title, dateTaken, loc, orientation, exif, mOnPhotoSavedListener,
					mContentResolver);
		}
		if (mMiniMode == MINI_MODE_WAITING_PHOTO && mMiniPictureTakenListener != null) {
			mMiniPictureTakenListener.onMiniPictureTaken(Storage.generateFilepath(title), mCameraId);
		}
	}
	...
	private String getWaterMarkString() {
		...
		if (mDesiredPreviewWidth == 1280) {
			...
			waterMark = (mDesiredPreviewWidth / 3) + ",15,0," + (mDesiredPreviewWidth / 8) + ", 15, ";
		} else if (mDesiredPreviewWidth == 1920) {
			Log.i("cdong", "getWaterMarkString QUALITY_1080P");
			waterMark = (mDesiredPreviewWidth / 2) + ",20,0," + (mDesiredPreviewWidth / 4) + ", 20, ";
		} else {
			Log.i("cdong", "getWaterMarkString QUALITY_1296P");
			waterMark = (mDesiredPreviewWidth / 2) + ",15,0," + (mDesiredPreviewWidth / 4) + ", 15, ";
		}
		...
		if (mCameraId == CameraInfo.CAMERA_FACING_FRONT && mDesiredPreviewWidth_src == 1280
			...
			waterMark = (mDesiredPreviewWidth_src / 3) + ",15,0," + (mDesiredPreviewWidth_src / 8) + ", 15, ";
		}
	...
	private String getGPStWaterMarkString() {
		...
		if (mCameraId == CameraInfo.CAMERA_FACING_FRONT && mDesiredPreviewWidth_src == 1280
				&& mDesiredPreviewWidth == 1920) {
			Log.i("cdong", "getWaterMarkString change to QUALITY_720P");
			waterMark = (mDesiredPreviewWidth_src / 3) + ",0,0," + (mDesiredPreviewWidth_src / 8) + ", 0, ";
		}
		return waterMark;
	}
