...
status_t AWRecorder::setCamera(const sp<ICamera>& camera, const sp<ICameraRecordingProxy>& proxy , int cameraId)
{
    ...
    status_t ret = mAWCarletRecorder->setCamera(camera, proxy, cameraId);
    if (OK != ret) {
...
status_t AWRecorder::setPreviewSurface(const sp<IGraphicBufferProducer>& surface)
{
    ...
    status_t ret = mAWCarletRecorder->setPreviewSurface(surface);
    if (OK != ret) {
...
status_t AWRecorder::init()
{
    ...
    status_t ret = OK;//mAWRecorder->init();
    if (OK != ret) {
    ...
    ret = OK;//mAWRecorder->setListener(this);
    if (OK != ret) {
...
status_t AWRecorder::setVideoSource(int vs)
{
    ...
    status_t ret = OK;//mAWRecorder->setVideoSource(vs);
...
status_t AWRecorder::setAudioSource(int as)
{
    ...
    status_t ret = OK;//mAWRecorder->setAudioSource(as);
    if (OK != ret) {
...
status_t AWRecorder::setOutputFormat(int of)
{
    ...
    status_t ret = mAWCarletRecorder->setOutputFormat(of);
    if (OK != ret) {
...
status_t AWRecorder::setVideoEncoder(int ve)
{
...
*/
    status_t ret = OK;//mAWRecorder->setVideoEncoder(ve);
    if (OK != ret) {
...
status_t AWRecorder::setAudioEncoder(int ae)
{
...
*/
    status_t ret = OK;//mAWRecorder->setAudioEncoder(ae);
    if (OK != ret) {
...
status_t AWRecorder::setOutputFile(const char* path)
{
...
*/
    status_t ret = mAWCarletRecorder->setOutputFile(path);
    if (OK != ret) {
...
status_t AWRecorder::setOutputFile(int fd, int64_t offset, int64_t length)
{
    ...
    */
    ...
    status_t ret = mAWCarletRecorder->setOutputFile(fd, offset, length);
    if (OK != ret) {
        ALOGV("setOutputFile failed: %d", ret);
        mCurrentState = MEDIA_RECORDER_ERROR;
        return ret;
...
status_t AWRecorder::setNextOutputFile(int fd, int64_t offset, int64_t length)
{
    ALOGV("setNextOutputFile(%d, %lld, %lld)", fd, offset, length);
    if (mAWRecorder == NULL) {
        ALOGE("media recorder is not initialized yet");
        return INVALID_OPERATION;
    }
    if (mIsNextOutputFileSet) {
        ALOGE("output file has already been set");
        return INVALID_OPERATION;
    }
    if (!(mCurrentState & MEDIA_RECORDER_DATASOURCE_CONFIGURED)) {
    ...
    status_t ret = OK;//mAWRecorder->setOutputFile(fd, offset, length);
    if (OK != ret) {
        ALOGV("setOutputFile failed: %d", ret);
        mCurrentState = MEDIA_RECORDER_ERROR;
        return ret;
    }
    mIsNextOutputFileSet = true;
    return ret;
}

status_t AWRecorder::outputToNextFileStart(int fd, int64_t offset, int64_t length)
{
    ALOGV("outputToNextFileStart(%d, %lld, %lld)", fd, offset, length);
    if (mAWRecorder == NULL) {
        ALOGE("media recorder is not initialized yet");
        return INVALID_OPERATION;
    }
    ...
    status_t ret = mAWCarletRecorder->outputToNextFileStart(fd, offset, length);
    if (OK != ret) {
        ALOGV("setOutputFile failed: %d", ret);
        mCurrentState = MEDIA_RECORDER_ERROR;
        return ret;
...
status_t AWRecorder::outputToNextFileStart(const String8& path)
{
    ALOGD("outputToNextFileStart(%s)", path.string());
    if (mAWRecorder == NULL) {
        ALOGE("media recorder is not initialized yet");
        return INVALID_OPERATION;
    }
    ...
    status_t ret = mAWCarletRecorder->outputToNextFileStart(path);
    if (OK != ret) {
        ALOGV("setOutputFile failed: %d", ret);
        mCurrentState = MEDIA_RECORDER_ERROR;
        return ret;
...
status_t AWRecorder::outputToNextFileStart()
{
    ALOGV("outputToNextFileStart(void)");
    if (mAWRecorder == NULL) {
        ALOGE("media recorder is not initialized yet");
        return INVALID_OPERATION;
    }
...
status_t AWRecorder::setVideoSize(int width, int height, int out_width, int out_height)
{
...
*/
    ...
    status_t ret = mAWCarletRecorder->setVideoSize(width, height, out_width, out_height);
    if (OK != ret) {
...
status_t AWRecorder::setMicMute(int micStatu)
{
    ...
    status_t ret = mAWCarletRecorder->setMicMute(micStatu);
    if (OK != ret) {
        ALOGE("setMicMute failed: %d", ret);
        mCurrentState = MEDIA_RECORDER_ERROR;
        return ret;
    }
...
sp<IGraphicBufferProducer> AWRecorder::querySurfaceMediaSourceFromMediaServer()
{
    Mutex::Autolock _l(mLock);
    mSurfaceMediaSource = NULL;//mAWRecorder->querySurfaceMediaSource();
...
status_t AWRecorder::setVideoFrameRate(int frames_per_second)
{
    ...
    status_t ret = OK;//mAWRecorder->setVideoFrameRate(frames_per_second);
    if (OK != ret) {
...
status_t AWRecorder::setParameters(const String8& params) {
    ...
    status_t ret = mAWCarletRecorder->setParameters(params);
    if (OK != ret) {
...
status_t AWRecorder::prepare()
{
    ...
    status_t ret = mAWCarletRecorder->prepare();
    if (OK != ret) {
...
status_t AWRecorder::getMaxAmplitude(int* max)
{
    ...
    status_t ret = OK;//mAWRecorder->getMaxAmplitude(max);
    if (OK != ret) {
...
status_t AWRecorder::start()
{
    ...
    status_t ret = mAWCarletRecorder->start();
    if (OK != ret) {
...
status_t AWRecorder::stop()
{
    ...
    status_t ret = mAWCarletRecorder->stop();
    if (OK != ret) {
...
status_t AWRecorder::close()
{
    ...
    status_t ret = OK;//mAWRecorder->close();
    if (OK != ret) {
...
status_t AWRecorder::doReset()
{
    ALOGV("doReset");
    status_t ret = OK;//mAWRecorder->reset();
    if (OK != ret) {
...
void AWRecorder::doCleanUp()
{
    ...
    mIsNextOutputFileSet = false;
}

// Release should be OK in any state
status_t AWRecorder::release()
{
    ALOGE("AWRecorder, release");
    status_t ret = OK;
    if (mAWCarletRecorder != NULL) {
        ret = mAWCarletRecorder->release();
        if(ret == OK) {
            delete mAWCarletRecorder;
            mAWCarletRecorder = NULL;
        }
        return ret;
    }
    return INVALID_OPERATION;
...
{
    ALOGE("AWRecord,constructor");

    /*const sp<IMediaPlayerService>& service(getMediaPlayerService());
    if (service != NULL) {
    	mAWRecorder = service->createMediaRecorder();
    }*/
    mAWRecorder = this;
	mAWCarletRecorder = new AWCarletRecorder();
    if (mAWRecorder != NULL) {
        mCurrentState = MEDIA_RECORDER_IDLE;
    }
...
status_t AWRecorder::initCheck()
{
    return mAWRecorder != 0 ? NO_ERROR : NO_INIT;
}

AWRecorder::~AWRecorder()
{
    ALOGE("AWRecord,destructor");
    if (mAWRecorder != NULL) {
    	//mAWRecorder.clear();
    }
...
status_t AWRecorder::setListener(const sp<AWRecorderListener>& listener)
{
    ALOGV("setListener");
    Mutex::Autolock _l(mLock);
    mListener = listener;
...
status_t AWRecorder::setClientName(const String16& clientName)
{
    ALOGV("setClientName");
    if (mAWRecorder == NULL) {
        ALOGE("media recorder is not initialized yet");
    ...
    //mAWRecorder->setClientName(clientName);

    return NO_ERROR;
}

void AWRecorder::notify(int msg, int ext1, int ext2)
{
    ALOGV("message received msg=%d, ext1=%d, ext2=%d", msg, ext1, ext2);

    sp<AWRecorderListener> listener;
    mLock.lock();
    listener = mListener;
    mLock.unlock();
