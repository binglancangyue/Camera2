...
#include "./libAWCarletRecorder/include/AWCarletRecorder.h"
...
class AWRecorderListener: virtual public RefBase
{
public:
    virtual void notify(int msg, int ext1, int ext2) = 0;
};

class AWRecorder : virtual public RefBase{
public:
	AWRecorder();
    ~AWRecorder();
    ...
    status_t    setListener(const sp<AWRecorderListener>& listener);
    ...
    status_t    close();
    status_t    release();
    status_t    setNextOutputFile(int fd, int64_t offset, int64_t length);
    status_t    outputToNextFileStart(int fd, int64_t offset, int64_t length);
	status_t    outputToNextFileStart(const String8& path);
    status_t    outputToNextFileStart();
    status_t    setMicMute(int enableMute);
...
private:
    ...
    sp<AWRecorder>          mAWRecorder;
    sp<AWRecorderListener>   mListener;

	AWCarletRecorder*	mAWCarletRecorder;//	mAWCarletRecorder;
