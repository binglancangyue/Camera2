...
public abstract class LocalMediaData implements LocalData {
    ...
    protected boolean imageDiaplyEnabled = true;
    ...
    public boolean delete(Context ctx) {
        ...
        Log.d(TAG, "&&&&&&&&&& camera adapter remove data from sdcard " +mPath);
        return f.delete();
    }
	...
	public void setImageDisplayed(boolean enable) {
		// TODO Auto-generated method stub
		Log.d(TAG, "video data setImageDisplayed " +enable);
		imageDiaplyEnabled = enable;
	}
    ...
    public static final class VideoData extends LocalMediaData {
        ...
        static VideoData buildFromCursor(Cursor c) {
            ...
            //Log.d(TAG, "local media data, build from cursor, path=" + path);
            ...
            try {
                ...
                //Log.d(TAG, "MEDIA retriever  build from cursor get video data use time " + (System.currentTimeMillis() - start));
		...
		private boolean isImpactVideo() {
			...
			String pathSuffix = mPath.substring(index - RecordService.IMPACT_SUFFIX.length(), index);
			if (pathSuffix.compareTo(RecordService.IMPACT_SUFFIX) != 0) {
				return false;
			}
			Log.i(TAG, "ImpactVideo, path=" + mPath);
			return true;
		}
        ...
        public View getView(final Activity activity,
			...
			if (isImpactVideo()) {
				...
				tv.setText(R.string.impact_protected_file);
        ...
        private final class VideoBitmapLoadTask extends BitmapLoadTask {
            ...
            protected Bitmap doInBackground(Void... v) {
                if (isCancelled() || !isUsing()) {
                	Log.d(TAG, "============ MEDIA retriever start getFrameAtTime isCancelled=" + isCancelled() + ",  isUsing=" + isUsing());
                    return null;
                }
                ...
                try {
                    ...
                    if ((!isCancelled() && isUsing())) {
                    	long start = System.currentTimeMillis();
                    	//Log.d(TAG, "============ MEDIA retriever start getFrameAtTime ===========");
                        //if (data != null) {
                         //   retriever.release();
                        //    Log.d(TAG, "MEDIA retriever get bitmap use time " + (System.currentTimeMillis() - start));
                      ...
                      //  if (bitmap == null) {
                        ...
                        //Log.d(TAG, "============MEDIA retriever get frame at time use time " + (System.currentTimeMillis() - start));
                       // }
                    }
