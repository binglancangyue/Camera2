...
public class CameraDataAdapter implements LocalDataAdapter {
    ...
    private class QueryTask extends AsyncTask<ContentResolver, Void, LocalDataList> {
        ...
        protected LocalDataList doInBackground(ContentResolver... resolver) {
            ...
            c = resolver[0].query(
                    ...
                    LocalMediaData.VideoData.QUERY_PROJECTION,
                    MediaStore.Video.Media.DATA + " like ? ", pat,
                    LocalMediaData.VideoData.QUERY_ORDER);
            if (c != null && c.moveToFirst()) {
                // build up the list.
                c.moveToFirst();
                while (true) {
                    ...
                    //Log.e(TAG, "get data:"+ c.getString(LocalMediaData.VideoData.COL_DATA));
                    if (data != null) {
                        l.add(data);
                    } else {
                        Log.e(TAG, "Error loading data:"
            ...
            //Log.d(TAG, c+";"+LocalMediaData.VideoData.CONTENT_URI);
            if (c != null) {
                c.close();
            }
    ...
    public static void cleanInvalidVideoFile(ContentResolver resolver) {
        ...
        if (c != null && c.moveToFirst()) {
            ...
            while (true) {
                ...
                if (data != null) {
                    ...
                    /*Log.e(TAG, "Got Invalid file:"
                            + c.getString(LocalMediaData.VideoData.COL_DATA));*/
                	MediaMetadataRetriever retriever = null;
                    int width = c.getInt(LocalMediaData.VideoData.COL_WIDTH);
                    int height = c.getInt(LocalMediaData.VideoData.COL_HEIGHT);
                    String path = c.getString(LocalMediaData.VideoData.COL_DATA);
                    try{
                        if (width == 0 || height == 0) {
                    ...
                    if (width == 0 || height == 0) {
                        ...
                        Log.d(TAG, "Got a Invalid file,going to delete it:" + path);
                        resolver.delete(LocalMediaData.VideoData.CONTENT_URI, MediaStore.Video.VideoColumns._ID + "=" + c.getString(LocalMediaData.VideoData.COL_ID), null);
