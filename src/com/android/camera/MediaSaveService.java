...
public class MediaSaveService extends Service {
    ...
    public void addVideo(String path, long duration, ContentValues values,
            OnMediaSavedListener l, ContentResolver resolver,boolean updateOnly) {
        // We don't set a queue limit for video saving because the file
        // is already in the storage. Only updating the database.
        new VideoSaveTask(path, duration, values, l, resolver,updateOnly).execute();
    }
    ...
    private class VideoSaveTask extends AsyncTask <Void, Void, Uri> {
        ...
        public VideoSaveTask(String path, long duration, ContentValues values,
                OnMediaSavedListener l, ContentResolver r,boolean updateOnly) {
            this.path = path;
            this.duration = duration;
            this.values = new ContentValues(values);
            this.listener = l;
            this.resolver = r;
            this.mUpdateOnly = updateOnly;
        }

        @Override
        protected Uri doInBackground(Void... v) {
        	Log.d(TAG, "VideoSaveTask" + values);
            values.put(Video.Media.SIZE, new File(path).length());
            values.put(Video.Media.DURATION, duration);
            Uri uri = null;
            Log.d(TAG, "VideoSaveTask path=" + path);
            if(path == null) {
            	return null;
            }
            File tmpFile = new File(path);
            Log.d(TAG, "VideoSaveTask tmpFile.exists()=" + tmpFile.exists());
            if(!tmpFile.exists()) {
            	return null;
            }
            if (tmpFile.length() < 1 * 1024 * 1024) {
            	tmpFile.delete();
            	return null;
            }
            try {
                ...
                if(mUpdateOnly) {
                	resolver.update(videoTable, values, Video.Media.DATA, new String[]{path});
                }else {
                    ...
                    if (new File(path).renameTo(new File(finalName))) {
                        path = finalName;
                        Log.d(TAG, "path=" +  path + ",  rename to " + finalName);
                    } else {
                    	Log.e(TAG, "path=" + path + "rename failed");
                    	resolver.delete(videoTable, Video.Media.DATA, new String[]{finalName});
                    }
                    
                    uri = resolver.insert(videoTable, values);
                    Log.d(TAG, "VideoSaveTask uri=" + uri);

                    if(uri != null) {
                    	Log.e(TAG, "finalName=" + finalName);
                    	resolver.update(uri, values, null, null);
                    } else {
                    	Log.e(TAG, "finalName2=" + finalName);
                    	resolver.update(videoTable, values, Video.Media.DATA, new String[]{finalName});
                    }
                }
            } catch (Exception e) {
