...
//import android.hardware.Camera.AWMoveDetectionListener;  // AWMoveDetection
...
class AndroidCameraManagerImpl implements CameraManager {
    ...
    private static final int SET_WATERMARK_FLAG = 510;
    private static final int SET_MOTIONDETECT_FLAG = 511;
    private static final int SET_MOTIONDETECT_LISTENER = 512;
    private static final int STOP_RENDER = 513;
    private static final int START_RENDER = 514;
	/* adas start */
    private static final int SET_ADAS_DETECTION_LISTENER = 661;
    private static final int START_ADAS_DETECTION =        662;
    private static final int STOP_ADAS_DETECTION =         663;
    private static final int SET_ADAS_ERROR_CALLBACK =     664;
    private static final int SET_ADAS_CAR_SPEED =		   665;
	private static final int SET_WATER_MARK = 			   666;
	private static final int SET_CAR_LINE_LEVEL =		   667;
    ...
    private class CameraHandler extends Handler {
		...
		/* adas start */
        private void startAdasDetection() {
        	//todo
            //mCamera.startAdasDetection();
        }

        private void stopAdasDetection() {
            //mCamera.stopAdasDetection();
        }

        private void setAdasDetectionListener(/*AdasDetectionListener listener*/) {
            //mCamera.setAdasDetectionListener(listener);
        }
		/* adas end */
            ...
            try {
                switch (msg.what) {
                    ...
                    case SET_PREVIEW_TEXTURE_ASYNC:
                        ...
                        return;
					case SET_MOTIONDETECT_LISTENER:
						//mCamera.setAWMoveDetectionListener((AWMoveDetectionListener)msg.obj);
						return;	
					case SET_MOTIONDETECT_FLAG:
						if (msg.arg1 == 1) {
							//mCamera.startAWMoveDetection();
						} else {
							//mCamera.stopAWMoveDetection();
						}
						return;
					case SET_WATERMARK_FLAG:
						if (msg.arg1 == 1) { 
                    ...
                    case STOP_FACE_DETECTION:
                        ...
                        return;
  /* adas start */
                    case SET_ADAS_DETECTION_LISTENER:
                        //setAdasDetectionListener((AdasDetectionListener) msg.obj);
                        return;

                    case START_ADAS_DETECTION:
                        startAdasDetection();
                        return;

                    case STOP_ADAS_DETECTION:
                        stopAdasDetection();
                        return;
                    /* adas end */
                    ...
                    case REFRESH_PARAMETERS:
                        mParametersIsDirty = true;
                        return;
                    case SET_ADAS_CAR_SPEED:
                    	if(msg.obj != null && msg.obj instanceof Float) {
                    		//mCamera.setADASCarSpeed((Float)msg.obj);
                    	}
                    	return;
					case SET_WATER_MARK:
						if(msg.obj != null && msg.obj instanceof String) {
							//mCamera.setWaterMarkMultiple(msg.obj.toString());
						}
						return;
					case SET_CAR_LINE_LEVEL:
						if(msg.obj != null && msg.obj instanceof Integer) {
							//mCamera.setLaneLineOffsetSensity((Integer)msg.obj);
						}
						break;
    ...
    public class AndroidCameraProxyImpl implements CameraManager.CameraProxy {
		...
		public void startRender() {
			mCameraHandler.obtainMessage(
                    START_RENDER, 0, 0).sendToTarget();
        }

		@Override
		public void stopRender() {
			mCameraHandler.obtainMessage(
                    STOP_RENDER, 0, 0).sendToTarget();
        }

		@Override
		public void setWaterMark(boolean enable) {
			mCameraHandler.obtainMessage(
                    SET_WATERMARK_FLAG, (enable ? 1 : 0), 0).sendToTarget();
        }

		@Override
		public void setMotionDetect(boolean enable) {
			mCameraHandler.obtainMessage(
                    SET_MOTIONDETECT_FLAG, (enable ? 1 : 0), 0).sendToTarget();
        }

		@Override
		public void setMotionDetectListener(/*AWMoveDetectionListener listener*/) {
            //mCameraHandler.obtainMessage(SET_MOTIONDETECT_LISTENER, listener).sendToTarget();
        }

		@Override
		public void setAdasDetectListener(/*AdasDetectionListener listener*/) {
            //mCameraHandler.obtainMessage(SET_ADAS_DETECTION_LISTENER , listener).sendToTarget();
        }
	...
	/* adas start */
        public void setAdasDetectionCallback(
                Handler handler, CameraAdasDetectionCallback cb) {
        	Log.v(TAG, "setAdasDetectionCallback");
            mCameraHandler.obtainMessage(
                    SET_ADAS_DETECTION_LISTENER,
                    AdasDetectionCallbackForward.getNewInstance(handler, this, cb)).sendToTarget();
        }

        @Override
        public void startAdasDetection() {
        	Log.v(TAG, "startAdasDetection");
            mCameraHandler.sendEmptyMessage(START_ADAS_DETECTION);
        }

        @Override
        public void stopAdasDetection() {
        	Log.v(TAG, "stopAdasDetection");
            mCameraHandler.sendEmptyMessage(STOP_ADAS_DETECTION);
        }
		/* adas end */
        @Override
		...
		@Override
		public void setADASCarSpeed(float speed) {
		...
		@Override
		public void setCarLineLevel(int level) {
			mCameraHandler.obtainMessage(
					SET_CAR_LINE_LEVEL,
		...
		@Override
		public void setWaterMarkMultiple(String mark) {
			mCameraHandler.obtainMessage(
				SET_WATER_MARK,
				mark
			).sendToTarget();
	...
	/* adas start */
    private static class AdasDetectionCallbackForward /*implements AdasDetectionListener*/ {
        private final Handler mHandler;
        private final CameraAdasDetectionCallback mCallback;
        private final CameraProxy mCamera;

        /**
         * Returns a new instance of {@link AdasDetectionCallbackForward}.
         *
         * @param handler The handler in which the callback will be invoked in.
         * @param camera  The {@link CameraProxy} which the callback is from.
         * @param cb      The callback to be invoked.
         * @return        The instance of the {@link AdasDetectionCallbackForward},
         *                or null if any parameter is null.
         */
        public static AdasDetectionCallbackForward getNewInstance(
                Handler handler, CameraProxy camera, CameraAdasDetectionCallback cb) {
            if (handler == null || camera == null || cb == null) return null;
            return new AdasDetectionCallbackForward(handler, camera, cb);
        }

        private AdasDetectionCallbackForward(
                Handler h, CameraProxy camera, CameraAdasDetectionCallback cb) {
            mHandler = h;
            mCamera = camera;
            mCallback = cb;
        }
        ...
        //@Override
        public void onAdasDetection(/*final Camera.Adas adas,*/ Camera camera) {
            mHandler.post(new Runnable() {
                @Override
                public void run() {
                    //mCallback.onAdasDetection(adas, mCamera);
                }
            });
        }
    }
