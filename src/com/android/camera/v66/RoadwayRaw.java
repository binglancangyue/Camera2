...
public class RoadwayRaw extends View {
    ...
    public RoadwayRaw(Context context, AttributeSet attrs) {
        super(context, attrs);
        mPaint = new Paint();
        mPath = new Path();
        mBounds = new Rect();
    ...
    public RoadwayRaw(Context context) {
        super(context);
        mPaint = new Paint();
        mPath = new Path();
        mBounds = new Rect();
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mAdas == null) {
            if (mInstallCalMode) {
                ...
                float leftUpY = midRow;
                float leftDnX = midCol - hLen;
                float leftDnY = midRow + vLen;
                float rightUpX = midCol;
                float rightUpY = midRow;
                float rightDnX = midCol + hLen;
                float rightDnY = midRow + vLen;
        ...
        if (mInstallCalMode) {
            ...
            float leftUpY = convertCoordinate(height, subH, midRow);
            float leftDnX = convertCoordinate(width, subW, midCol - hLen);
            float leftDnY = convertCoordinate(height, subH, midRow + vLen);
            float rightUpX = convertCoordinate(width, subW, midCol);
            float rightUpY = convertCoordinate(height, subH, midRow);
            float rightDnX = convertCoordinate(width, subW, midCol + hLen);
            float rightDnY = convertCoordinate(height, subH, midRow + vLen);
